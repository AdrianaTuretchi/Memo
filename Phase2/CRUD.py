...
def update_checked_status(conn,item_id,checked):
    cursor = conn.cursor()
    cursor.execute("UPDATE checklist_items SET checked = ? WHERE id = ?", (checked, item_id))
    conn.commit()
def update_checked_note(conn,note_id,new_item):
    cursor = conn.cursor()
    cursor.execute("INSERT INTO checklist_items (content, note_id) VALUES (?, ?)", new_item, note_id)
    conn.commit()
